...
const InventoryManager: React.FC<InventoryManagerProps> = ({ inventory, setInventory, globalPaints }) => {
  ...
  // Use globalPaints if provided, fallback to COMMON_PAINTS
  const availablePaints = globalPaints.length > 0 ? globalPaints : COMMON_PAINTS;
  ...
  const handleUpdatePaintColor = (paintId: string, newHex: string) => {
    // Update in user inventory if it exists there
