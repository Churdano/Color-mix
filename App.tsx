...
import { doc, getDoc, setDoc, onSnapshot, collection, getDocs, orderBy, query } from 'firebase/firestore';
...
const MainContent: React.FC = () => {
  ...
  const [globalPaints, setGlobalPaints] = useState<Paint[]>(COMMON_PAINTS);
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      if (user) {
        setIsGuest(false);
        setAuthUid(user.uid);
        setIsAdmin(user.email === 'gonzalink88@gmail.com');
  ...
  useEffect(() => {
    if (isAuthReady) {
      const loadGlobalPaints = async () => {
        try {
          const q = query(collection(db, 'admin_colors'), orderBy('createdAt', 'desc'));
          const snap = await getDocs(q);
          const adminPaints: Paint[] = [];
          snap.forEach(d => {
            const data = d.data();
            adminPaints.push({
              id: data.refId || d.id,
          ...
          setGlobalPaints([...COMMON_PAINTS, ...adminPaints]);
        } catch (err) {
          console.error('Error loading global paints:', err);
          setGlobalPaints(COMMON_PAINTS);
        }
      };
      loadGlobalPaints();
    }
        ...
        <>
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        {isAdmin && (
                          <button 
                              ...
                              className="p-2 rounded-lg bg-white/5 hover:bg-purple-500/10 border border-transparent hover:border-purple-500/30 text-stone-400 hover:text-purple-400 transition-all duration-300"
                              title="Admin Panel"
                          >
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <main className="relative z-10 max-w-4xl mx-auto px-4 py-10 space-y-8">
                ...
                <section className="animate-fade-in-up transition-transform hover:scale-[1.01] duration-500" style={{animationDelay: '0.1s'}}>
                    <InventoryManager inventory={currentUser.inventory} setInventory={setUserInventory} globalPaints={globalPaints} />
                </section>
